...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv1(x)
        #total_flops = 0
        #total_flops += get_flops(list(x.size()), list(out.size()))
        ...
        out = self.conv2(out)
        #total_flops += get_flops(s, list(out.size()))
        out = self.bn2(out)
        '''with open("flops_measure.log", "a") as f:
            f.write("Total FLops per layer :" + str(total_flops) + "\n")
        f.close()'''
...
class RNNGate(nn.Module):
    ...
    def forward(self, x):
        ...
        #inputs.append(x.squeeze().tolist()[0])
...
class ResNetRecurrentGateSP(nn.Module):
    ...
    def forward(self, x):
        ...
        mask, gprob, prob1 = self.control(gate_feature)
        A = torch.tensor(32)
        gprobs.append(gprob)
        tensor_list = []
        tensor_list2 = []
        #print(gprob)
        tensor_list.append(gprob[0])
        tensor_list2.append(gprob[1])
        probs.append(prob1[0])
        probs2.append(prob1[1])
        masks.append(mask.squeeze())
        prev = x  # input of next layer
        temp = 0
        ...
        stacked_tensor2 = torch.stack(tensor_list2)

        # return x, masks, gprobs
        
        return probs, probs2, stacked_tensor, stacked_tensor2
...
class RNNGatePolicy(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            self.saved_actions.append(action)
        action = action.view(action.size(0), 1, 1, 1).float()
        prob1=bi_prob.tolist()
        return action, bi_prob,prob1
...
class ResNetRecurrentGateRL(nn.Module):
    ...
    def forward(self, x):
        ...
        stacked_tensor2 = torch.stack(tensor_list2)
        #return x, masks, gprobs
        #print("gprobs ",numpy.asarray(gprobs).shape)
        #print(probs)
        #print("-----------")

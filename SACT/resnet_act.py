...
def get_halting_proba(outputs):
  with tf.variable_scope('halting_proba'):
    ...
    halting_proba, flops,total_flops = flopsometer.conv2d(
        x,
        1,
        1,
    ...
    return halting_proba, flops,total_flops
...
def get_halting_proba_conv(outputs, residual_mask=None):
  with tf.variable_scope('halting_proba'):
    ...
    halting_logit, current_flops,total_flops = flopsometer.conv2d(
        local_feature,
        1,
        SACT_KERNEL_SIZE,
        activation_fn=None,
    ...
    halting_logit_global, current_flops,total_flops = flopsometer.conv2d(
        global_feature,
        1,
        1,
        activation_fn=None,
    ...
    return halting_proba, flops,tot_flops
...
def unit_act(block,
  ...
  with tf.variable_scope('unit_%d' % (unit_idx + 1), [inputs]):
    outputs, flops,tot_flops = block.unit_fn(
        inputs, *block.args[unit_idx], residual_mask=residual_mask)

    if not skip_halting_proba and unit_idx < len(block.args) - 1:
      if sact:
        halting_proba, current_flops,total_flops = get_halting_proba_conv(
            outputs, residual_mask)
        flops += current_flops
        tot_flops+=total_flops
      else:
        halting_proba, current_flops,total_flops = get_halting_proba(outputs)
        flops += current_flops
        tot_flops += total_flops
    else:
      halting_proba = None

    return outputs, halting_proba, flops,tot_flops
...
def stack_blocks(net, blocks, model_type, end_points=None):
  ...
  end_points['total_flops'] = end_points.get('total_flops', 0)
  end_points['block_scopes'] = [block.scope for block in blocks]
  end_points['block_num_units'] = [len(block.args) for block in blocks]
  e_points = {}
  e_points['flops'] = end_points.get('flops', 0)
  e_points['total_flops'] = end_points.get('total_flops', 0)
  e_points['block_scopes'] = [block.scope for block in blocks]
  ...
  for block in blocks:
    if act_func:
      (ponder_cost, num_units, flops,tot_flops, halting_distribution, net) = act_func(
          net,
          partial(unit_act, block, sact=(model_type == 'sact')),
          len(block.args),
          scope=block.scope)
      e_points['{}/ponder_cost'.format(block.scope)] = ponder_cost
      ...
      with tf.variable_scope(block.scope, 'block', [net]):
        ...
        for unit_idx in range(len(block.args)):
          net, _, current_flops,total_flops = unit_act(
              block, net, unit_idx, skip_halting_proba=True)
          flops += current_flops
          tot_flops+=total_flops
    end_points['{}/flops'.format(block.scope)] = flops
    end_points['flops'] += flops
    e_points['{}/flops'.format(block.scope)] = flops
    e_points['flops'] += flops
    end_points['{}/total_flops'.format(block.scope)] = tot_flops
    end_points['total_flops'] += tot_flops
    e_points['{}/total_flops'.format(block.scope)] = tot_flops
    e_points['total_flops'] += tot_flops
    end_points[block.scope] = net

  return net, end_points,e_points

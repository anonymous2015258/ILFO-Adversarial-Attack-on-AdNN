...
def bottleneck(inputs,
  ...
  with tf.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
    ...
    if depth == depth_in:
      ...
      shortcut, current_flops,total_flops = flopsometer.conv2d(
          preact,
          depth, [1, 1],
          stride=stride,
          normalizer_fn=None,
    ...
    residual, current_flops,total_flops = flopsometer.conv2d(
        preact,
    ...
    flops += current_flops
    tot_flops += total_flops
    residual, current_flops,total_flops = flopsometer.conv2d_same(
        residual,
        depth_bottleneck,
        3,
        stride,
    ...
    return outputs, flops,tot_flops
...
def resnet_v2(inputs,
  ...
  with tf.variable_scope(scope, 'resnet_v2', [inputs], reuse=reuse) as sc:
    ...
    with slim.arg_scope([slim.conv2d], activation_fn=None, normalizer_fn=None):
      net, current_flops,total_flops = flopsometer.conv2d_same(
          net, 64, 7, stride=2, scope='conv1')
      end_points['flops'] += current_flops
      end_points['total_flops']=total_flops
    net = slim.max_pool2d(net, [3, 3], stride=2, scope='pool1')
    # Early stopping is broken in distributed training.
    net, end_points,e_points = resnet_act.stack_blocks(
        net,
        blocks,
    ...
    if num_classes is not None:
      net, current_flops,total_flops = flopsometer.conv2d(
          net,
          num_classes, [1, 1],
          activation_fn=None,
          normalizer_fn=None,
...
def get_network(images,
  ...
  logits, end_points ,e_points= resnet_v2(
      images,
      blocks,
      num_classes,
  ...
  return logits, end_points,e_points

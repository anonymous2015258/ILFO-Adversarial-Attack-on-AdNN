...
def spatially_adaptive_computation_time(inputs, unit, max_units,
  ...
  with tf.variable_scope(scope):
    ...
    for unit_idx in range(max_units):
      ...
      if not unit_idx:
        (state, halting_proba, flops,tot_flops) = unit(
        ...
        (state, halting_proba, current_flops,total_flops) = unit(
  ...
  return (ponder_cost, num_units, flops,tot_flops, halting_distribution, outputs)
